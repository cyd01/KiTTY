...
static void sk_net_close(Socket *sock) { return sk_tcp_close( *sock ) ; }
#else
...
void select_result(WPARAM wParam, LPARAM lParam)
{
...
#ifdef MOD_ZMODEM
	if( (!GetPuttyFlag()) && GetZModemFlag() &&0 ) {
	if( !s->closing ) {
	if (ret < 0) {
	    plug_closing(s->plug, winsock_error_string(err), err, 0);
	} else if (0 == ret) {
	...
	do {
		...
		if (err == WSAEWOULDBLOCK)
		    break;
#ifdef MOD_ZMODEM
		if( (!GetPuttyFlag()) && GetZModemFlag() && s->closing )
			return ;
#endif
		plug_closing(s->plug, winsock_error_string(err), err, 0);
#ifdef MOD_ZMODEM
	...
	return;
      case FD_ACCEPT: {
		...
		if( (!GetPuttyFlag()) && GetZModemFlag() ) {
			if (!s->closing && plug_accepting(s->plug, sk_net_accept, actx)) {
			p_closesocket(t);      /* denied or error */
		}
	    }
	else {
		if (plug_accepting(s->plug, sk_net_accept, actx)) {
		p_closesocket(t);      /* denied or error */
		}
	    }
#else
